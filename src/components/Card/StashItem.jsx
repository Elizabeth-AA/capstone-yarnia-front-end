...
const StashItem = ({ stash }) => {
    return (
        <div className="flex justify-center md:flex-row md:flex-wrap">
            {stash.map((item) => (
                <div key={item.rav_id} className="card card-compact card-side flex items-start bg-primary relative h-28 w-3/4">
                    <figure><img className="h-28" src={item.photo.small_url} alt="skein of yarn" /></figure>
